...
pub struct Character {
    ...
    pub likes_dislikes: String,
    pub inner_world: String,
...
pub async fn upsert_character_in_pool(
    ...
    sqlx::query(
        ...
        INSERT INTO characters
          (id, project_id, character_type, name, aliases_json, role, short_description, appearance, temperament, likes_dislikes, inner_world, worldview, secret, voice_notes, mannerisms, origin, family, background, knowledge_notes, visual_prompt, image_asset_id, status, order_index, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
    ...
    .bind(input.likes_dislikes)
    .bind(input.inner_world)
    .bind(input.worldview)
    .bind(input.secret)
    .bind(input.voice_notes)
    .bind(input.mannerisms)
    .bind(input.origin)
    .bind(input.family)
    ...
    async fn character_workspace_persists_relations_memories_and_links() {
        ...
        let hero = upsert_character_in_pool(
            ...
            UpsertCharacterInput {
                ...
                temperament: "".into(),
                likes_dislikes: "".into(),
                inner_world: "Dazy do: Odnalezc mape\nPotrzebuje: Zaufac komus".into(),
                worldview: "".into(),
                secret: "".into(),
                voice_notes: "".into(),
                mannerisms: "".into(),
